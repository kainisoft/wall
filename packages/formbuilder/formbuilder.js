...
(function($) {
    ...
    var FormBuilder = function(element, options) {
        ...
        var appendFieldLi = function(title, field, values) {
            ...
            li += '<li id="frm-' + lastID + '-item" class="' + values.type + ' form-field" data-type="' + values.type + '">';
            li += '<div class="legend">';
            li += delBtn;
                ...
                $(this).children().each(function() {
                    if (!($(this).hasClass('moving') || $(this).hasClass('disabled'))) {
                        for (var att = 0; att < opts.attributes.length; att++) {
                            ...
                            json.optional = !$('#' + $(this).attr('id') + ' input.required').is(':checked');
                            json.presentaion = $(this).data('type');
                            json.label = $('#' + this.id + ' input.fld-label').val();
                            json.desc = $('#' + this.id + ' input.fld-description').val();
                            var optional = $('#' + $(this).attr('id') + ' input.required').is(':checked') ? 'required="true" ' : 'required="false" ';
                            var multipleChecked = $('#' + $(this).attr('id') + ' input[name="multiple"]').is(':checked');
                            var multiple = multipleChecked ? 'style="multiple" ' : '';
                            var t = $(this).attr(opts.attributes[att]); // field type
                            var type = 'type="' + t + '" ';
                            var fID = $(this).attr('id');
                            var fName = 'name="' + $('#' + fID + ' input.fld-name').val() + '" ';
                            var fLabel = 'label="' + $('#' + fID + ' input.fld-label').val() + '" ';
                            var roleVals = $.map($('#' + fID + ' input.roles-field:checked'), function(n) {
                                    return n.value;
                                }).join(',');
                            var roles = (roleVals !== '' ? 'role="' + roleVals + '" ' : '');
                            var desc = 'description="' + $('#' + fID + ' input.fld-description').val() + '" ';
                            var maxLengthVal = $('#' + fID + ' input.fld-max-length').val();
                            var maxLength = 'max-length="' + (maxLengthVal !== undefined ? maxLengthVal : '') + '" ';
                            var fSlash = (t !== '6' && t !== '10' ? '/' : '');

                            serialStr += '\n\t\t<field ' + fName + fLabel + multiple + roles + desc + (maxLengthVal !== '' ? (maxLengthVal !== undefined ? maxLength : '') : '') + optional + type + fSlash + '>';
